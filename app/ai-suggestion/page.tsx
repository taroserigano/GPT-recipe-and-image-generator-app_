...
const AiSuggestion = () => {
  ...
  }, [recipe, suggestions, wordLimit, language]);
  ...
  const saveRecipe = () => {
    if (typeof window !== 'undefined') {
      ...
      const updatedrecipeData = recipeData.map((recipeData: any) =>
        recipeData.content === recipe ? { ...recipeData, recipe: data } : recipeData
      );
  ...
  return (
    <div className="container mx-auto p-4 dark:bg-gray-900 dark:text-white transition duration-300">
      <ToastContainer />
      <h1 className="text-3xl font-bold mb-6 text-center">Recipe Screen for {recipe} 😉</h1>

      <div className="text-center mb-6">
        <label htmlFor="wordLimit" className="mr-2">Word Limit:</label>
        <input
          id="wordLimit"
          type="number"
          value={wordLimit}
        ...
        <label htmlFor="language" className="mr-2 ml-4">Language:</label>
        <select
          id="language"
          value={language}
